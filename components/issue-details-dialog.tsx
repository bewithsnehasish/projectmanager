...
interface User {
  ...
  email: string;
  imageUrl: string;
  clerkUserId: string; // Assuming clerkUserId exists
}
...
interface IssueDetailsDialogProps {
  ...
  issue: Issue;
  onDelete?: () => void;
  onUpdate?: (updatedIssue: any) => void; // Keep as any as requested
  borderCol?: string;
}
          ...
          <div className="flex items-center space-x-2">
            <Select value={status} onValueChange={handleStatusChange}>
              ...
              <SelectContent>
                {statuses.map(
                  (
                    option: { key: string; name: string }, // added type to status map
                  ) => (
